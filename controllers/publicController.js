...
const getPosts = async (req,res) => {
    try {
        ...
        FROM (SELECT CONCAT(first_name, ' ', last_name) as fullName, id FROM users) as sub 
        INNER JOIN posts ON author = sub.id WHERE status = '1' AND is_headline = 'false' AND 'One Cikanlar' = ANY(posts.categories) ORDER BY created_date DESC`)
        ...
        const categories = await queries(`SELECT * FROM categories WHERE status = '1' ORDER BY created_date ASC`)
        res.json({result: "success", data: {headline: headline.rows, lastNews: lastNews.rows, bitcoin: bitcoin.rows, otherNews: otherNews.rows, categories: categories.rows}})
    } catch (error) {
